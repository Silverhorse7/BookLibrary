...
const BookController = {
  ...
  addCategory(req, res, next) {
    return BookCategory.findOrCreate({ where: { category: req.body.category } })
      .then((result) => {
        const category = result[0]; // This is the instance
        const created = result[1]; // This is a boolean indicating if the instance was created
        const { id } = category;
        return created
          ? res.status(201).send({
          ...
          : res.status(409).send({
              message: `Category, ${category.category}, already exists`,
            });
      })
  ...
  returnBook(req, res, next) {
    ...
    BorrowedBook.findOne({ where: { userId, bookId, returned: false } })
      .then((borrowedBook) => {
        ...
        return res.status(403).send({
          message:
            "This book is currently not on your list. You have either returned it or never borrowed it",
        });
      })
      .catch((error) => next(error));
  },
